        ...
        if (relocations) |relocs| {
            for (relocs) |reloca| {
                const reloc_addr_bytes = blk: {
                    if (reloca.size == 1) {
                        break :blk byte.intToLEBytes(u8, @intCast(u8, secs.items[reloca.sec.idx].vaddr + reloca.ofst_in_sec - secs.items[text_idx].vaddr - main_entry - reloca.loc - reloca.size));
                    } else if (reloca.size == 2) {
                        break :blk byte.intToLEBytes(u16, @intCast(u16, secs.items[reloca.sec.idx].vaddr + reloca.ofst_in_sec - secs.items[text_idx].vaddr - main_entry - reloca.loc - reloca.size));
                    } else if (reloca.size == 4) {
                        break :blk byte.intToLEBytes(u32, @intCast(u32, secs.items[reloca.sec.idx].vaddr + reloca.ofst_in_sec - secs.items[text_idx].vaddr - main_entry - reloca.loc - reloca.size));
                    } else if (reloca.size == 8) {
                        break :blk byte.intToLEBytes(u64, @intCast(u64, secs.items[reloca.sec.idx].vaddr + reloca.ofst_in_sec - secs.items[text_idx].vaddr - main_entry - reloca.loc - reloca.size));
                    }
