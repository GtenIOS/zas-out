...
pub const Segment = struct {
        ...
        return blk: {
            ...
            for (secs) |sec| {
                if (sec.data) |sec_data| {
                    sec_data_size += sec_data.items.len;
                }
                if (include_reserve) {
                    if (sec.res_size) |res_size| {
                        sec_data_size += res_size;
                    }
                }
            }
    ...
    pub fn initFromText(secs: std.ArrayList(Section), align_size: u16, ovlp_ofst: u64, offset: u64, vaddr: u64) Self {
    ...
    pub fn initFromData(secs: std.ArrayList(Section), align_size: u16, offset: u64, vaddr: u64) Self {
        if (secs.items.len == 0) @panic("Segment should atleast contain one section");
        const sec_data_size = totalSecSize(secs.items, false);
        var padding_bytes_size: u16 = 0;
